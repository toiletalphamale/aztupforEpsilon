...
function library.notify(message, time, color)
    ...
    task.spawn(function()
        ...
        local sizetween = TweenService:Create(notification.accentBottomBar, TweenInfo.new(time, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = UDim2.new(1,0,0, 1)})
        sizetween:Play()
        local postween = TweenService:Create(notification.background, TweenInfo.new(1.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(0,0,0,0)})
        postween:Play()
        task.wait(time);
...
function library.update_notifications()
    ...
    for v in next, library.notifications do
        local anim = TweenService:Create(v.holder, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0,20,0, 75 + (i * 30))})
        anim:Play()
        i = i + 1
    end
end
function library.update_notifications2(drawing) -- i hate this i hate this...
    local postween = TweenService:Create(drawing.background, TweenInfo.new(1.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(0, -(drawing.background.Size.X.Offset + 10), 0, drawing.background.Position.Y)})
    postween:Play()

    for i,v in pairs(drawing) do
        if type(v) == "table" then
            local transtween = TweenService:Create(v, TweenInfo.new(1, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Transparency = 0})
            transtween:Play()
        end
    end
end
